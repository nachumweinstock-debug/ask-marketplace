...
// Shorten full university name to a familiar label
function uniLabel(university) {
  if (!university) return 'your campus';
  const map = {
    'Yeshiva University': 'YU',
    'Stern College for Women': 'Stern',
    'Sy Syms School of Business': 'Syms',
    'Albert Einstein College of Medicine': 'Einstein',
    'Cardozo School of Law': 'Cardozo',
    'Wurzweiler School of Social Work': 'Wurzweiler',
    'Azrieli Graduate School': 'Azrieli',
    'Ferkauf Graduate School': 'Ferkauf',
  };
  return map[university] || university;
}

export function buildShareText(link, university) {
  const school = uniLabel(university);
  return `Hey! ASK is a campus app to find OR offer tutoring, barbers, fitness and more at ${school}. Sign up with my link: ${link}`;
}
...
export default function SharePanel({ referralCode, university }) {
  ...
  const text = useMemo(() => buildShareText(link, university), [link, university]);
...
function solidLink(disabled) {
  return { ...base, background: '#fff', color: '#1B3A6B', opacity: disabled ? 0.45 : 1, pointerEvents: disabled ? 'none' : 'auto' };
}
