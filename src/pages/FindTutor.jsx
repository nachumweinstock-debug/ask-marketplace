...
const SCHOOLS = ['Yeshiva University', 'NYU', 'Baruch College', 'Columbia University', 'Rutgers University', 'UCLA', 'USC', 'Boston University'];
...
export default function FindTutor() {
  ...
  async function submit(e) {
    ...
    setLoading(true);
    setSearched(true);
    trackEvent('ai_match_started', { subject: form.subject, school: form.school, urgency: form.urgency, mode: form.mode });
    try {
      const params = {
        category: 'tutor',
      ...
      const ranked = (data || []).map(provider => ({
        ...
        matchScore:
          ...
          (provider.college?.toLowerCase() === form.school.toLowerCase() ? 20 : 0) +
    ...
    <div className="page" style={{ maxWidth: 1120 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: 22, alignItems: 'start' }}>
        <div className="card" style={{ padding: 24, borderRadius: 18, position: 'sticky', top: 84 }}>
          <div className="section-label">Tutor matching</div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 38, lineHeight: 1, marginTop: 8 }}>Find the right tutor</h1>
          <p style={{ fontSize: 14, color: 'var(--muted)', lineHeight: 1.6, margin: '10px 0 18px' }}>
            Tell Ask what class you are stuck on and we will rank tutors by subject fit, school fit, ratings, availability, price, and response signals.
          </p>
          <form onSubmit={submit} style={{ display: 'grid', gap: 11 }}>
            <Field label="School">
              <input list="schools" value={form.school} onChange={e => setForm(f => ({ ...f, school: e.target.value }))} style={input} />
          ...
          {loading ? (
            ...
            <>
              <div className="card" style={{ padding: 18, marginBottom: 14, borderRadius: 16 }}>
                <strong>Why these tutors?</strong>
                <p style={{ color: 'var(--muted)', fontSize: 13, lineHeight: 1.6, marginTop: 5 }}>
                  Ranked for {form.subject} at {form.school}, within about ${form.budget}, with stronger trust signals and relevant specialties first.
                </p>
              </div>
            ...
            <div className="card" style={{ padding: 48, textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--muted)', marginTop: 8 }}>Try a broader subject or higher budget. We also recommend browsing all tutors.</p>
            </div>
          ) : (
