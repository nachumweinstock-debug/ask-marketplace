...
router.get('/', requireAuth, (req, res) => {
  const user = db.prepare(
    'SELECT id, email, name, username, role, is_admin, avatar_url, university, major, interests, classes_taking, gpa, user_bio, zelle, venmo, phone, contact_pref, timezone, referral_code, referred_by, created_at FROM users WHERE id = ?'
  ).get(req.user.id);
...
router.put('/', requireAuth, async (req, res) => {
  try {
    const { name, username, university, major, interests, classes_taking, gpa, user_bio, avatar_data_url, zelle, venmo, phone, contact_pref, timezone } = req.body;
    const userUpdates = {};

    if (name?.trim())                 userUpdates.name           = name.trim().slice(0, 100);
    if (university !== undefined)     userUpdates.university     = String(university || '').slice(0, 120);
    ...
    const ALLOWED_COLS = new Set(['name','username','university','major','interests','classes_taking','gpa','user_bio','zelle','venmo','phone','contact_pref','avatar_url','timezone']);
    ...
    const updated = db.prepare(
      'SELECT id, email, name, username, role, is_admin, avatar_url, university, major, interests, classes_taking, gpa, user_bio, zelle, venmo, phone, contact_pref, timezone, referral_code, created_at FROM users WHERE id = ?'
    ).get(req.user.id);
    res.json(updated);
  } catch (err) {
