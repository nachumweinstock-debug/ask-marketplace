...
// GET /providers/price-stats?category=tutor
router.get('/price-stats', (req, res) => {
  const { category } = req.query;
  if (!category) return res.status(400).json({ error: 'category required' });
  const stats = db.prepare(`
    SELECT COUNT(*) as count, MIN(price_per_session) as min,
           MAX(price_per_session) as max, ROUND(AVG(price_per_session)) as avg
    FROM provider_profiles
    WHERE price_per_session > 0 AND (category = ? OR custom_category = ?)
...
router.get('/', (req, res) => {
  ...
  if (search) {
    query += ' AND (u.name LIKE ? OR pp.bio LIKE ? OR pp.custom_category LIKE ? OR pp.title LIKE ?)';
...
async function updateProfile(req, res, profile) {
  try {
    const { bio, category, price_per_session, zelle, venmo, custom_category, listing_image_data_url, session_type, title } = req.body;
    ...
    db.prepare(`
      ...
      SET bio = ?, category = ?, price_per_session = ?, zelle = ?, venmo = ?,
          custom_category = ?, listing_image = ?, session_type = ?, title = ?
      WHERE id = ?
    `).run(
      bio ?? profile.bio,
